...
function init() {
  if (!window.questions) {
    console.error("❌ Erro: trueorfalse.js não foi carregado!");
    return;
  }
...
function loadQuestion(index) {
  ...
  const options = [
    { text: "Certo", value: true },
    { text: "Errado", value: false }
  ];
  ...
  options.forEach(opt => {
    ...
    if (answered) {
      if (opt.value === answered.selectedValue) {
        div.classList.add(answered.correct ? 'correct' : 'incorrect');
      }
      if (!answered.correct && opt.value === q.correct) {
        div.classList.add('correct');
      }
    } else {
      div.addEventListener('click', () => checkAnswer(div, opt, q, index));
    }
  ...
  if (answered) {
    ...
    feedbackEl.textContent = (answered.correct ? '✅ ' : '❌ ') + q.explanation;
    feedbackEl.style.color = answered.correct ? '#63ff9e' : '#ff7b7b';
  }
}

function checkAnswer(optionEl, opt, q, questionIndex) {
  ...
  if (opt.value === q.correct) {
    ...
    feedbackEl.textContent = "❌ " + q.explanation;
    errors++;
    const correctDiv = Array.from(optionNodes).find(o => o.textContent === (q.correct ? "Certo" : "Errado"));
